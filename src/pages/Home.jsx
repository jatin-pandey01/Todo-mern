...
const Home = () => {
  const {todo,setTodo,id,name,email,createTodo,setCreateTodo} = useContext(TodoContext);
  ...
  useEffect(()=>{
    ...
    const apiCall = async()=>{
      try {
        ...
        console.log(data);
        const res2 = await axios.post('https://todo-api-kax0.onrender.com/api/v1/todo/get-all-todo',{
          email:data.email,
        });
  ...
  const deleteClick = async(todoId)=>{
    try {
      const res = await axios.post('https://todo-api-kax0.onrender.com/api/v1/todo/delete-todo',{
        userId:user.id,
        todoId:todoId,
      });
      console.log(res);
        ...
        document.cookie ? <div>
          {
            ...
            }) : <div className='h-[80vh] flex justify-center items-center flex-col gap-2'>
              <p className='text-2xl font-semibold text-white'> Dear {name}, Please create todo  </p>
              <p className='text-xl bg-green-600 text-white px-4 py-[5px] rounded-md cursor-pointer' onClick={()=>{setCreateTodo(true)}}> Create </p>
            </div>
          }
